...
def explain_lime(model, volume_3d, device, target_class=None):
    ...
    slice_lime = (slice_lime - slice_lime.min()) / (
        slice_lime.max() - slice_lime.min() + 1e-8
    )
...
def explain_integrated_gradients_stream(model, volume_3d, device, target_class=None):
    ...
    slice_map = (slice_map - slice_map.min()) / (
        slice_map.max() - slice_map.min() + 1e-8
    )
...
def predict_knee_diagnosis(
    model, device, volume_3d: np.ndarray, target_shape=(32, 128, 128)
):
    if model is None:
        return None, {}
    ...
    vol_tensor = (
        torch.tensor(vol, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    )  # (1,1,D,H,W)
    vol_resized = torch.nn.functional.interpolate(
        vol_tensor, size=target_shape, mode="trilinear", align_corners=False
    )  # (1,1,32,128,128)
...
def main():
    ...
    if uploaded is not None:
        ...
        if arr is None:
            st.write(
                "Nie znaleziono tablicy 2D/3D w pliku .pck — pokażę reprezentację:"
            )
            ...
            with col1:
                ...
                if arr.ndim == 2:
                    ...
                    mid = arr.shape[0] // 2
                    st.image(
                        _normalize_for_display(arr[mid]),
                        caption=f"Środkowy przekrój (slice {mid})",
                        use_column_width=True,
                    )
            ...
            if model is not None and device is not None:
                if st.button("🚀 Uruchom analizę", key="diagnose_btn"):
                    with st.spinner("Analizuję obraz..."):
                        ...
                        logger.info(
                            f"Diagnosis prediction done. Class: {predicted_class}"
                        )
            ...
            if st.session_state.ai_result:
                ...
                with col_prob1:
                    st.metric(
                        "Zdrowe (0)", f"{probs['Zdrowe kolano (0)']:.2%}", delta=None
                    )
                with col_prob2:
                    st.metric(
                        "ACL Częściowe (1)",
                ...
                with xai_col1:
                    ...
                    if (
                        st.session_state.xai_method == "lime"
                        and st.session_state.xai_result is None
                    ):
                    ...
                    if (
                        ...
                        and st.session_state.xai_result[0] == "LIME"
                    ):
                        st.image(
                            st.session_state.xai_result[1],
                            caption="LIME (middle slice)",
                            use_column_width=True,
                        )
                ...
                if st.session_state.xai_result:
                    ...
                    fig, ax = plt.subplots()
                    ax.imshow(
                        _preprocess_for_model(arr)
                        .squeeze(0)
                        .squeeze(0)[arr.shape[0] // 2],
                        cmap="gray",
                    )
                    attributions_mask = img >= img.max() * 0.3
